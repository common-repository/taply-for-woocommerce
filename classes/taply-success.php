...
class Taply_Success extends WC_Payment_Gateway {
    ...
    public function success(){  
        $strApiUrl = 'https://api.taplycheckout.com/payment/';
        ...
        $strMerchantId = $dst_settings['merchant_id'];
        ...
        curl_setopt($process, CURL_SSLVERSION_TLSv1_2, 1);
        curl_setopt($process, CURL_SSLVERSION_TLSv1_1, 1);
        curl_setopt($process, CURLOPT_FOLLOWLOCATION, 1); 
        ...
        $arrOrderCart = is_array($sOrderCartJson) ? $sOrderCartJson : json_decode($sOrderCartJson, TRUE);
        $arrOrderTransaction =  is_array($sOrderTransaction) ? $sOrderTransaction : json_decode( $sOrderTransaction, TRUE);
        WC()->cart->empty_cart();
